...
const TopRatedPage = () => {
  ...
  const { data: topRatedMovies, isLoading } = useQuery({
    queryKey: ['GET_TOP_RATED_MOVIES', page],
    queryFn: async () => {
  ...
  return (
    <>
      <div className='grid grid-cols-3 gap-2 px-3 md:px-5 lg:px-10 md:grid-cols-4 md:gap-3 lg:grid-cols-5 2xl:grid-cols-10 lg:gap-5 top-28 left-0 right-0 absolute -z-10'>
        {topRatedMovies?.map((movie: MovieInterface) => (
          <CardMovie
            key={movie.id}
            title={movie.title}
