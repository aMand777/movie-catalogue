...
const HeaderCardMoviesHome: React.FC<HeaderCardMoviesHomeProps> = ({ children, title, link }) => {
  ...
  return (
    ...
    <div className='flex justify-between items-center mx-1 md:mx-3 mt-5'>
      <Badge variant='solid' colorScheme='red' className='rounded-tr-lg cursor-default text-sm'>{children}</Badge>
      <Link href={link}>
        <span className='flex text-viewAll text-xs lg:text-sm hover:text-white items-center'>
